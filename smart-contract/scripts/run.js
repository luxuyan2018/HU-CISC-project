...
async function logNftLists(marketplace) {
  ...
  const accountAddress = "0xB42570C50f6061E4e10F3211c6826ed48a06a964";
...
const main = async (cb) => {
  try {
    ...
    await marketplace.resellNft(boredPets.address, tokenId2, 1, {
      value: listingFee,
    });
    await logNftLists(marketplace);
  } catch (err) {
